...
export default class Door extends Phaser.GameObjects.Container
{
    ...
    constructor (name:string, scene:GamePlay, x:number, y:number)
    {
        super(scene, x, y);

        this.name = name;

        this.background = scene.add.image(0, 0, 'bank-panic', 'doorBackground');
        this.door = scene.add.sprite(0, 0, 'bank-panic', 'door1');
        this.character = scene.add.image(0, 0, 'bank-panic', 'bandit1');
    ...
    shootCharacter (closeDoor:boolean)
    {
        this.isDead = true;
